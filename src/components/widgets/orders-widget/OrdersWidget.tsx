...
export const OrdersWidget: FC = () => {
	const [notPaid, ] = useState(orders.filter(order => order.paid === false).length);
	const [notDelivered, ] = useState(orders.filter(order => order.delivered === false).length);
	const [refunds, ] = useState(orders.filter(order => order.returned === false).length);
	...
	return (
		<div className="border border-solid border-gray-400 p-4 rounded-xl w-full h-full">
			...
			<div className={'flex gap-1 mt-10 justify-between items-center px-4'}>
				<OrderTypeCounter image={NotPaidImage} numberOfElements={notPaid} header={'NotPaid'} target={RoutesConstants.NOT_PAID_ORDERS_PAGE} />
				<OrderTypeCounter image={NotSentImage} numberOfElements={notDelivered} header={'NotSent'} target={RoutesConstants.NOT_DELIVERED_ORDERS_PAGE} />
				<OrderTypeCounter image={RefundImage} numberOfElements={refunds} header={'Refunds'} target={RoutesConstants.RETURNED_ORDERS_PAGE} />
			</div>
		</div>
	);
};
